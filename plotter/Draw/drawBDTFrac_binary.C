...
// echo 'gROOT->LoadMacro("drawBDTFrac_binary.C+"); gSystem->Exit(0);' | root -b -l
// rootbq 'drawBDTFrac_binary.C("v30", "DY PU200", "PU200-DYToLL_M50", "L1Tk")'
// rootbq 'drawBDTFrac_binary.C("v30", "DY PU200", "PU200-DYToLL_M50", "")'

void drawBDTFrac_binary(
  ...
  TString Dir = "../plot_Binary/plots_"+ver+"/plots_BDTFrac_"+ver+"/"+tag+"/";
  ...
  for(int ivar=0; ivar<(int)v_var.size(); ++ivar) {
    ...
    c->SaveAs(Dir+canvasName+logy_tag+"_sort10.pdf","pdf");
    c->SaveAs(Dir+canvasName+logy_tag+"_sort10.png","png");
